    ...
    <div className="relative">
      <div className="flex gap-1">
        <input
          ...
          placeholder="Müşteri ara veya isim yaz…"
          value={query}
        ...
        />
        {selected && (
          <button onClick={clear} className="px-2 text-gray-400 hover:text-red-500">✕</button>
        )}
      </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="border-t border-gray-100 pt-3 space-y-1 text-sm">
            ...
            <div className="flex justify-between text-gray-500 pt-1">
              <span>Ödeme</span>
              <span className={`badge ${METHOD_BADGE[sale.payment_method]}`}>{sale.payment_method}</span>
            </div>
          </div>
...
// ── Main Sales page (inline POS + history) ─────────────────
export default function Sales() {
  ...
  // Load products on search/filter change
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-col" style={{ maxHeight: '38%' }}>
                ...
                : sales.map(s => (
                  <tr key={s.id} className="hover:bg-gray-50 cursor-pointer transition-colors"
                    ...
                    <td className="px-4 py-2">
                      <span className={`badge text-xs ${METHOD_BADGE[s.payment_method]}`}>{s.payment_method}</span>
                    </td>
